...
def get_bus():
    ...
    # Si l'utilisateur n'a rien mis, on utilise tes coordonnées par défaut
    lat = request.args.get('lat', '48.8328')
    lon = request.args.get('lon', '2.2527')
    
    # On insère les coordonnées variables dans l'URL
    url = f"https://prim.iledefrance-mobilites.fr/marketplace/v2/navitia/coords/{lon};{lat}/stop_schedules?line_id=line:IDFM:C01375"
    ...
    try:
        ...
        if not data.get('stop_schedules'):
             return jsonify({"markup": "⚠️ Pas d'horaires (Vérifiez GPS)"})
